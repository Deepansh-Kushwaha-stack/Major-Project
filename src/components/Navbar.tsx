...
import { Menu, X, ChevronDown, User, Zap, Sparkles } from 'lucide-react';
    ...
    <>
          ...
          <div className="flex justify-between items-center">
            <button onClick={() => handleNavigation('/')} className="flex items-center space-x-3 group">
              <div className="relative">
                {/* New Animated Logo - Hexagon with Lightning */}
                <div className="w-12 h-12 bg-gradient-to-br from-yellow-400 via-amber-400 to-orange-500 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-all duration-300 group-hover:rotate-12 shadow-lg relative overflow-hidden animate-pulse">
                  {/* Main Icon */}
                  <div className="relative w-8 h-8 flex items-center justify-center">
                    <Sparkles className="w-6 h-6 text-stone-800 animate-spin" style={{ animationDuration: '3s' }} />
                    {/* Orbiting elements */}
                  ...
                  {/* Animated border */}
                  <div className="absolute inset-0 border-2 border-stone-800/30 rounded-2xl animate-pulse"></div>
                  {/* Glow effect */}
                  <div className="absolute -inset-1 bg-gradient-to-br from-yellow-400/50 to-orange-400/50 rounded-2xl blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 animate-pulse"></div>
                </div>
              </div>
      ...
      {isOpen && (
        <div className="md:hidden fixed inset-0 z-40 animate-fadeIn">
          ...
          {/* Mobile Navigation Content - Positioned below navbar with proper spacing */}
          <div className="relative flex flex-col h-full overflow-y-auto">
            {/* Spacer to push content below navbar */}
            <div className="h-24 flex-shrink-0"></div>
            
            {/* Navigation content with padding and scrollable area */}
            <div className="flex-1 px-6 py-8 space-y-6 min-h-0">
